...
function handleCheckEmail(email) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.getSheetByName('Registrations');
  if (!sheet || sheet.getLastRow() < 2) return responseJSON({ exists: false });
  var emails = sheet.getRange(2, 3, sheet.getLastRow() - 1, 1).getValues().flat();
  return responseJSON({ exists: emails.indexOf(email) !== -1 });
}
...
function handleSubmitQuiz(data) {
  ...
  lbSheet.appendRow([data.email.split('@')[0], data.score, data.timestamp]);
