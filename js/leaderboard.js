...
const urlTimeSpent = urlParams.get('timeSpent');
...
async function fetchLeaderboard() {
    ...
    try {
        ...
        if (currentUserEmail) {
            ...
            if (existingIndex !== -1) {
                ...
                if (localScore > (allData[existingIndex].score || 0)) {
                    ...
                    allData[existingIndex].timeSpent = parseFloat(localStorage.getItem('mercy_quiz_timeSpent') || urlTimeSpent || 999999);
                }

                // Clear the optimistic flag since we found them
                localStorage.removeItem('mercy_quiz_score');
                localStorage.removeItem('mercy_quiz_timeSpent');
                ...
                const tempUser = {
                    ...
                    score: localScore,
                    timeSpent: parseFloat(localStorage.getItem('mercy_quiz_timeSpent') || urlTimeSpent || 999999),
                    time: "Just now"
                };
            ...
            allData.sort((a, b) => {
                ...
                return (a.timeSpent || 999999) - (b.timeSpent || 999999);
            });
        }
        ...
        // Final Re-sort: Higher Score first, then lower time spent
        allData.sort((a, b) => {
            const scoreA = parseFloat(a.score || a.Score || 0);
            const scoreB = parseFloat(b.score || b.Score || 0);
            if (scoreB !== scoreA) return scoreB - scoreA;

            const timeA = parseFloat(a.timeSpent || 999999);
            const timeB = parseFloat(b.timeSpent || 999999);
