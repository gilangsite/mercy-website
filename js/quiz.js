...
async function finishQuiz(auto = false) {
    ...
    const timeSpentMs = Date.now() - (quizState.endTime - (QUIZ_DURATION_MINUTES * 60 * 1000));
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            window.location.href = `leaderboard.html?email=${encodeURIComponent(quizState.email)}&name=${encodeURIComponent(quizState.name)}&score=${finalScore}&timeSpent=${timeSpentSeconds}`;
        }, 1000);
